...
class Model(PyroModule):
    ...
    def __init__(self, config):
        ...
        self.latent_dim_t = config["latent_dim_t"]
        self.latent_dim_y = config["latent_dim_y"]
        self.binfeats = list(range(config["binary_dim"]))
        self.contfeats =list(range(config["continuous_dim"]))
    ...
    def z_dist(self,x):
        ...
        #check
        o1 = dist.Normal(loc, scale).to_event(1)
        return dist.Normal(loc, scale).to_event(1)

    def zt_dist(self):
        o2 =dist.Normal(0, 1).expand([self.latent_dim_t]).to_event(1)
