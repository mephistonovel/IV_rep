...
# from models import Autoencoder, VaDEIV
...
class Oursvt:
    ...
    def train(self):
        ...
        if self.args.pretrain==True:
            ...
            # 여기 주의 #  prior params만 남기고 업데이트 안하는 방식도 있음 self.args.use_flex_enc=False일 때 
...
class VaDEIV(torch.nn.Module):    
    ...
    def outcome(self, t,zc,):
        ...
        output: (t=1 net의 mu, sigma), (t=0Net의 mu, sigma)
        '''
    ...
    def forward(self, x,t,y):
        ...
        if self.use_dist_net and self.use_reg_net:
            ...
            return x_hat, mu_zc, mu_zt, prior_mu_zt, log_var_zc, log_var_zt, prior_log_var_zt, zc,zt,zt_prior, logit_t, y_sample, t_rep,t_sample
        
        else:        
            return x_hat, mu_zc, mu_zt, prior_mu_zt, log_var_zc, log_var_zt, prior_log_var_zt, zc,zt,zt_prior
    
    
    def compute_loss(self,x,t,y):
        if self.use_dist_net and self.use_reg_net:
            x_hat, mu_zc, mu_zt, prior_mu_zt, log_var_zc, log_var_zt, prior_log_var_zt, zc,zt,zt_prior, logit_t, y_sample, t_rep,t_sample = self.forward(x,t,y)        
        else:
            x_hat, mu_zc, mu_zt, prior_mu_zt, log_var_zc, log_var_zt, prior_log_var_zt, zc,zt,zt_prior = self.forward(x,t,y)
        ...
        if self.use_dist_net and self.use_reg_net:
            if self.treatment_type =='b':
                ...
                y1_mu = y_sample[0]
                y0_mu = y_sample[1]
                y_mse1= y1@torch.log(y1_mu+1e-5)+(1-y1)@torch.log(1-y1_mu+1e-5)
                y_mse0 = y0@torch.log(y0_mu+1e-5)+(1-y0)@torch.log(1-y0_mu+1e-5) 
                y_mse = -y_mse0[0]-y_mse1[0]
                # F.binary_cross_entropy(y1_mu.squeeze(),y1,reduction='sum')+F.binary_cross_entropy(y0_mu.squeeze(),y0,reduction='sum')        
    ...
    def sigmoid_CE_loss(self,y,logits):
        '''
        https://www.tensorflow.org/api_docs/python/tf/nn/sigmoid_cross_entropy_with_logits 
        '''
        # max(logit,0)
        maxpart = torch.sum(F.relu(logits))
        
        # y*logit
        dotproduct = torch.dot(logits.squeeze(),y)
        
        #log(1+exp(-abs(logit)))
        logpart = torch.sum(torch.log(1+torch.exp(-torch.abs(logits))))
        
        return maxpart-dotproduct+logpart 
