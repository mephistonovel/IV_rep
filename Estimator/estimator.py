...
def estimate_report(treatment, response, true_effect, zt_train, t_train, y_train, 
    ...
    for est in Est2.keys():
        ...
        if treatment=='b':
            ...
            # if est in ['DML','Ortho','IVGMM','tsls']:
            #     estimates[train_key] = effect_train
            #     estimates[test_key] = effect_test
            # else:
            #     t1_index =[i for i, element in enumerate(t_train) if element == 1]

            #     t0_index = [i for i, element in enumerate(t_train) if element == 0]
            #     t1_idx_test = [i for i, element in enumerate(t_test) if element == 1]
            #     t0_idx_test = [i for i, element in enumerate(t_test) if element == 1]
            #     estimates[train_key] = np.mean(effect_train[t1_index])-np.mean(effect_train[t0_index])
            #     estimates[test_key] = np.mean(effect_test[t1_idx_test])-np.mean(effect_test[t0_idx_test])
        else:
...
def estimate(treatment, response, true_effect, zt_train, t_train, y_train, 
             zt_test, t_test, y_test, x_condition_train=None, x_condition_test=None):
    estimates = {}
    for est in Est.keys():
